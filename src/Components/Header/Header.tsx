            ...
            <Link to={'/'}>
                <div className={'flex items-center'}>
                    <img alt={'logo'}
                         ...
                         src={metadata.logo} className={'w-12 md:w-16'}/>
                    <div className={'px-2'}/>
            ...
            <div className={'hidden lg:grid grid-flow-col auto-cols-max items-center gap-2'}>
                {
                    options.map((option, index) => {
                        ...
                        return <ActionComponent key={index} action={option.action}>
                            <span
                                className={'text-white text-lg cursor-pointer text-center rounded-full transition duration-500 ease-in-out p-2 hover:text-black hover:bg-white'}>
                                {option.title[lang]}
                            </span>
                        </ActionComponent>
                    })
